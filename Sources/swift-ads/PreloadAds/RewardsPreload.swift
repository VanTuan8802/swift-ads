//
//  RewardsPreload.swift
//  AdsSwift
//
//  Created by VanTuan8802 on 13/8/26.
//
//  Rewarded Ad Loader using Ad Preloading V2 (Preview headers)
//
...
/// Rewarded Ad Loader using Ad Preloading V2 (Preview headers)
public final class RewardsPreload: NSObject, @unchecked Sendable {

    public static let shared = RewardsPreload()
    ...
    /// Start/refresh preloading for a rewarded ad unit ID.
    ...
    public func isAvailable(adUnitId: String) -> Bool {
        return RewardedAdPreloader.shared.isAdAvailable(with: adUnitId)
    }

    /// Poll and show the next available rewarded ad for the given adUnitId.
    ///
    /// - Parameters:
    ///   - adUnitId: Rewarded ad unit id (also used as preloadID)
    ///   - onDismiss: called when ad is dismissed, receives wasRewarded
    ...
    public func show(adUnitId: String,
        ...
        guard let ad = RewardedAdPreloader.shared.ad(with: adUnitId) else {
        ...
        // 4) Present - RewardedAd requires userDidEarnRewardHandler
...
extension RewardsPreload: PreloadDelegate {
...
extension RewardsPreload: FullScreenContentDelegate {
    ...
    public func adDidDismissFullScreenContent(_ ad: FullScreenPresentingAd) {
        ...
        if store.buffer(for: entry.adUnitId) == 0 {
            RewardedAdPreloader.shared.stopPreloadingAndRemoveAds(for: entry.adUnitId)
        }
